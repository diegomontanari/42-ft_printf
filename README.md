...
Recursion can be complex to understand at first, so here is an example of how the process works to print the number `5248` in base `16`.
...
1. **Initial Call:**
   ...
   ```
   - `5248 >= 16`, so it calls itself with `5248 / 16 = 328`.

2. **Recursive Call 1:**
   ```c
   ft_print_nbr_base(328, base, count);
   ```
   - `328 >= 16`, so it calls itself with `328 / 16 = 20`.

3. **Recursive Call 2:**
   ```c
   ft_print_nbr_base(20, base, count);
   ```
   - `20 >= 16`, so it calls itself with `20 / 16 = 1`.
